...
async def before_crypto_agent(callback_context: CallbackContext) -> None:
    ...
    logger.info(
        "[AGENT_START] -> Entering agent domain: crypto_specialist | "
        "session=%s",
        ctx.session.id,
    )
...
async def after_crypto_agent(callback_context: CallbackContext) -> None:
    ...
    logger.info(
        "[AGENT_COMPLETE] -> crypto_specialist executed successfully. Latency: %ss",
        latency,
    )
